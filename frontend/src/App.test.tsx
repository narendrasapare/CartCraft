...
const products = [
  {
    ...
    price: 1899,
    imageUrl: '/images/products/everyday-backpack.webp',
    categoryId: 2,
  },
  {
    id: 2,
...
describe('App', () => {
  ...
  it('loads catalogue products and persists an item in the bag', async () => {
    vi.stubGlobal(
      ...
      vi.fn().mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url === '/api/carts' && init?.method === 'POST') {
          return {
            ok: true,
            json: async () => ({ id: 'cart-1', items: [], totalQuantity: 0, subtotal: 0 }),
          }
        }
    ...
    expect(
      await screen.findByRole('button', { name: 'Shopping bag with 1 item' }),
    ).toBeInTheDocument()
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url === '/api/auth/csrf')
          return {
            ok: true,
            json: async () => ({ headerName: 'X-CSRF-TOKEN', token: 'identity-token' }),
          }
  ...
  it('changes and remembers the interface language', async () => {
    ...
    expect(await screen.findByRole('button', { name: 'खाता' })).toBeInTheDocument()
    expect(localStorage.getItem('cartcraft.language')).toBe('hi')
    expect(document.documentElement.lang).toBe('hi')

    await user.selectOptions(languageSelector, 'te')
