...
export const createCart = async (): Promise<Cart> => readCart(await fetch('/api/carts', { method: 'POST' }))
...
export const setCartItemQuantity = async (cartId: string, productId: number, quantity: number): Promise<Cart> =>
  readCart(await fetch(`/api/carts/${cartId}/items/${productId}`, {
    method: 'PUT',
...
export const removeCartItem = async (cartId: string, productId: number): Promise<Cart> =>
  readCart(await fetch(`/api/carts/${cartId}/items/${productId}`, { method: 'DELETE' }))
